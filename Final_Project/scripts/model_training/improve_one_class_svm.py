...
"""
Network Anomaly Detection - One-Class SVM Improvement
------------------------------------------
This script improves the One-Class SVM model through hyperparameter tuning
...
def evaluate_model(name, model, X_test, y_test):
    ...
    logger.info(f"Prediction time for {len(X_test)} samples: {prediction_time:.2f} seconds")
...
# Subset of data for faster tuning and training
logger.info("Creating subset of data for training...")
# One-Class SVM is computationally expensive, so use a smaller subset
...
logger.info("Starting hyperparameter tuning for One-Class SVM...")

# Define parameter grid
param_grid = {
    'kernel': ['rbf', 'linear', 'poly'],
    'nu': [0.1, anomaly_ratio * 0.5, anomaly_ratio * 0.8],
...
selected_combinations = param_combinations[:9]  # Adjust this number based on computational resources
...
for i, result in enumerate(results[:5]):
    logger.info(f"{i+1}. F1: {result['f1_score']:.4f}, Accuracy: {result['accuracy']:.4f}, Training time: {result['training_time']:.2f}s, Params: {result['params']}")

# Train best model on full subset
logger.info(f"Training best model with parameters: {best_params}")
best_full_model = OneClassSVM(
...
best_full_model.fit(X_train_subset)

# Evaluate best model
best_metrics = evaluate_model("One-Class SVM (Optimized)", best_full_model, X_test, y_test)

# Save best model
joblib.dump(best_full_model, os.path.join(MODELS_DIR, 'one_class_svm_optimized.joblib'))
...
plt.title('Best Parameters for One-Class SVM')
plt.ylabel('Parameter Value')
plt.tight_layout()
plt.savefig(os.path.join(VISUALIZATIONS_DIR, 'one_class_svm_best_params.png'), dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(VISUALIZATIONS_DIR, 'one_class_svm_hyperparameter_tuning.png'), dpi=300)
plt.close()

# Training time comparison
plt.figure(figsize=(10, 6))
df_results['combination'] = range(1, len(df_results) + 1)
plt.bar(df_results['combination'], df_results['training_time'], color='green')
plt.title('Training Time by Parameter Combination')
plt.xlabel('Parameter Combination')
plt.ylabel('Training Time (seconds)')
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig(os.path.join(VISUALIZATIONS_DIR, 'one_class_svm_training_time.png'), dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(VISUALIZATIONS_DIR, 'one_class_svm_pca_visualization.png'), dpi=300)
plt.close()
...
logger.info(f"Prediction Time: {best_metrics['prediction_time']:.2f} seconds")

logger.info("\nOne-Class SVM optimization completed successfully.") 